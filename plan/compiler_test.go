...
r := 0
	...
	for _, testCase := range testCases {
		...
		vars := newVars()
		if testCase.variableName != "" {
			vars.SetValue(testCase.variableName, testCase.variable)
		}
		execution.Exec(vars)
		actual, err := vars.Value(testCase.resultPath)
		if !assert.Nil(t, err, testCase.description) {
			continue
		}
...
func initForBench() {
	...
	var err error
	benchExecution, newVars, err = scope.Compile(code)
	if err != nil {
		panic(err)
	}
	benchVars = newVars()
...
var benchLoopIgoVars *exec.Variables

func initLoopIgo() {
	scope := NewScope()
	scope.RegisterFunc("print", testPrintln)
	var varsNew exec.New
	var err error
	benchLoopIgo, varsNew, err = scope.Compile(`count :=0
for i :=0;i<100;i++ {
	count += i
}
print(count)	
