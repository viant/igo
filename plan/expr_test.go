...
func TestScope_IntExpression(t *testing.T) {
	...
	var testCase = []struct {
		...
		},
		{
			...
			expr:        "x+y-z",
			vars:        map[string]interface{}{"x": 10, "y": 20, "z": 5},
			expect:      25,
		},
		{
			description: "simple expr */ literal",
			expr:        "5*x/y*z",
			vars:        map[string]interface{}{"x": 10, "y": 25, "z": 5},
			expect:      10,
		},
		{
			description: "simple expr */ literal",
		...
		},
		{
			description: "<<",
			expr:        "x<<y",
			vars:        map[string]interface{}{"x": 1, "y": 3},
			expect:      8,
		},
		{
			description: ">>",
			expr:        "x>>y",
			vars:        map[string]interface{}{"x": 16, "y": 2},
			expect:      4,
		},
		{
			description: "struct x+y",
			expr:        "foo.X + foo.Y",
			vars:        map[string]interface{}{"foo": Foo{X: 2, Y: 3}},
			expect:      5,
		},
	}
	...
	for _, testCase := range testCase {
		...
		for k, v := range testCase.vars {
			if iV, ok := v.(int); ok {
				err = anExpr.Vars.SetInt(k, iV)
				assert.Nil(t, err)
			} else {
				err = anExpr.Vars.SetValue(k, v)
				assert.Nil(t, err)
			}
		}
...
func TestScope_BoolExpression(t *testing.T) {
	...
	var testCase = []struct {
	...
	}{
		{
			description: "err != nil",
			expr:        "err != nil",
			vars:        map[string]interface{}{"err": io.EOF},
			expect:      true,
		},
		{
			description: "x > y",
...

		{
			description: "x == y",
			expr:        "x == y",
			vars:        map[string]interface{}{"x": 10.0, "y": 10.0},
			expect:      true,
		},
		{
			description: "x >= y",
			expr:        "x >= y",
			vars:        map[string]interface{}{"x": 9.0, "y": 10.0},
			expect:      false,
		},
		{
			description: "foo != nil",
			expr:        "foo != nil",
			vars:        map[string]interface{}{"foo": &Foo{X: 1}},
			expect:      true,
		},
		{
			description: "foo == nil",
			expr:        "foo == nil",
			vars:        map[string]interface{}{"foo": fooPtr},
			expect:      true,
		},
		{
			description: "err == nil",
			expr:        "err == nil",
			vars:        map[string]interface{}{"err": err},
			expect:      true,
		},
		{
			description: `name == ""`,
		...
		},
		{
			description: `name != ""`,
			expr:        `name != ""`,
			vars:        map[string]interface{}{"name": "abc"},
			expect:      true,
		},
	}
	...
	for _, testCase := range testCase {
		...
		for k, v := range testCase.vars {
			if iV, ok := v.(string); ok {
				err = anExpr.Vars.SetString(k, iV)
				assert.Nil(t, err)
			} else {
				err = anExpr.Vars.SetValue(k, v)
				assert.Nil(t, err)
...
func TestScope_StringExpression(t *testing.T) {
	...
	var testCase = []struct {
	...
	}{
		{
			...
			expr:        "x+y",
			vars:        map[string]interface{}{"x": "abc", "y": "def"},
			expect:      "abcdef",
		},
		{
			description: "basic concatenation",
			expr:        "foo.X+y",
			vars:        map[string]interface{}{"foo": Foo{X: "this is "}, "y": "test"},
			expect:      "this is test",
		},
	}
...
func TestScope_Float64Expression(t *testing.T) {
	...
	var testCase = []struct {
	...
	}{
		{
			description: "basic +",
			expr:        "x+y",
			vars:        map[string]interface{}{"x": 1.4, "y": 3.4},
			expect:      4.8,
		},
		{
			description: "struct +",
			expr:        "foo.X+y",
			vars:        map[string]interface{}{"foo": Foo{X: 2.2}, "y": 2.2},
			expect:      4.4,
		},
	}
	...
	for _, testCase := range testCase {
		...
		for k, v := range testCase.vars {
			if iV, ok := v.(float64); ok {
				err = anExpr.Vars.SetFloat64(k, iV)
				assert.Nil(t, err)
			} else {
				err = anExpr.Vars.SetValue(k, v)
				assert.Nil(t, err)
